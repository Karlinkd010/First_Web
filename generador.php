...
require('conexion.php'); //cambiar a su conexion
	...
	$query="SELECT * FROM tblalumnos"; // su consulta a la tabla que ustedes deseen
...
/* seleccionamos el tipo, estilo y tamaño de la letra a utilizar */
		...
		$pdf->Write (7,"Reporte de Alumnos  ");
		$pdf->Ln();
		$pdf->Write (7, $row['txtNombre']);
		$pdf->Ln(); //salto de linea
		$pdf->Write (7, $row['txtAp']);
		$pdf->Ln(); //salto de linea 
		$pdf->Write (7, $row['txtAm']);
		$pdf->Ln(); //salto de linea
		$pdf->Write (7, $row['txtDomicilio']);
		$pdf->Ln(); //salto de linea
		$pdf->Write(7, $row['txtTelefono'],1,0,'C');
		$pdf->Ln(15);//ahora salta 15 lineas 
		$pdf->SetTextColor('255','0','0');//para imprimir en rojo 
		$pdf->Multicell(190,7, $row['intidalumnos']."\n Matricula",1,'R');
