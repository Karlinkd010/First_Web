...
$claves=$_GET['intidMaterias'];

if(isset($_GET['intidMaterias']))
{
  $sql="SELECT*FROM tblmaterias WHERE intidMaterias=$claves";
  $resultado=mysqli_query($conexion,$sql);
  if(mysqli_num_rows($resultado)>0)
  {
    $row=mysqli_fetch_assoc($resultado);

  $_matri=$row['intidMaterias'];
  $nombrema=$row['txtNombremateria'];
  $combocursoo=$row['intidprofesor'];
  $comprofeso0r=$row['intidCurso'];
...
<nav>
  ...
  <form id="form1" name="form1" method="POST" action="updatemateria.php">
    <h1>Registrar materias</h1>
     <br>
      <label>Matricula materia  </label><input type="text" value="<?=$_matri?>" name="id"  size "22" maxlength="11" required a>
      <br>
       <label>Nombre de la materia  </label><input type="text" value="<?=$nombrema?>" name="nam" size "22" required pattern="[A-Z a-z]{3,20}" title="Solo se aceptan letras">
      <br>
