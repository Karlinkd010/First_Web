...
$claves=$_GET['intidturno'];

if(isset($_GET['intidturno']))
{
  $sql="SELECT*FROM tblturno WHERE intidturno=$claves";
  $resultado=mysqli_query($conexion,$sql);
  if(mysqli_num_rows($resultado)>0)
  {
    $row=mysqli_fetch_assoc($resultado);

    $clave=$row['intidturno'];
    $turno=$row['txtDescripcionturno'];
...
<TD STYLE="WIDTH:50PX"><img src="imagenes/logo.png" wiDTH="200" height="160"></TD>
 ...
 <table>
  ...
  <th>
    </th><td><strong><font size="6">Edita datos turno</font></strong> <br>
    </td>
...
<nav>
  ...
  <form id="form1" name="form1" method="post" action="updateturno.php">
    <h1>Registrar cursos</h1>
     <br>
    <label>Id</label><input type="text" name="id" value="<?=$clave?>"  size "22" maxlength="11" required   >
      <br>

      <label>Turno</label><input type="text" name="turno" value="<?=$turno?>"  size "22" required  maxlength="50" pattern="[A-Z a-z]{3,20}" title="Solo se aceptan letras">
      <br>
